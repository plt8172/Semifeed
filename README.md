...
- `설정 (RSS 목록)` 노드의 `instagram_user_id`, `instagram_api_version`, `instagram_media_base_url`을 실제 값으로 변경합니다. `instagram_media_base_url`은 `Instagram 이미지 공개 Webhook` 노드에 표시되는 외부 HTTPS Production URL입니다.
...
4. `설정 (RSS 목록)`에서 아래 값을 변경합니다.
   - `instagram_user_id`: 위 API 응답의 `user_id`
   - `instagram_api_version`: Meta 앱에 표시된 지원 버전(기본값 `v25.0`)
   - `instagram_media_base_url`: `Instagram 이미지 공개 Webhook`의 HTTPS Production URL
