...
- Chrome 렌더링 직후 task runner가 일시적으로 응답하지 않아도 버티도록 heartbeat 간격을 300초로 확대
- 실행 시간대 기반 출처 라운드로빈을 적용하고 실행당 최대 카드 수를 5개로 확대
- 여러 카드의 browserless 렌더링을 한 장씩 순차 처리하도록 변경
